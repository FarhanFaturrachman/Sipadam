...
class InputGawatDaruratActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        locationCallback = object : LocationCallback() {
            override fun onLocationResult(result: LocationResult) {
                ...
                if (location != null) {
                    ...
                    Toast.makeText(this@InputGawatDaruratActivity, "Lokasi tidak tersedia", Toast.LENGTH_SHORT).show()
                }
        ...
        val kegiatanList = listOf("Mengantar Pasien Ke Pasilitas Kesehatan", "Penanggulangan Korban Lalalantas")
        etJenisKegiatan.setAdapter(ArrayAdapter(this, android.R.layout.simple_dropdown_item_1line, kegiatanList))
        ...
        val defaultUpaya = """
            1. Menerima Laporan
            2. Mempersiapkan sarpras penunjang dan evakuasi
            3. Mengevakuasi korban menggunakan sling baja dikaitkan ke rangka mobil dan ditarik dengan mobil lainnya, dan dinyatakan selesai pada pukul  (masukan waktu) selanjutnya anggota kembali ke mako pusat
            4. Membuat laporan
        ...
        // Generate checkbox petugas (contoh)
    ...
    // Fungsi menambahkan form korban dinamis
    private fun tambahFormKorban() {
        ...
        spinnerJenis.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                if (position == 0) spinnerJenis.setBackgroundColor(Color.parseColor("#FFCDD2"))
                else spinnerJenis.setBackgroundColor(Color.TRANSPARENT)
                isFormChanged = true
            }
    ...
    private fun setupFormChangeListeners() {
        val allInputs = listOf<View>(
            ...
            etAlamat, etKabupaten, etKecamatan, etDesa, etJarak, etKoordinatDesimal, etKoordinatGeografi,etKorbanSelamat, etKendaraan, etRencana, etKronologi, etApd, etUpaya, etPetugas
        )

        allInputs.forEach { view ->
            when (view) {
                is EditText -> view.addTextChangedListener { isFormChanged = true }
                is AutoCompleteTextView -> view.setOnItemClickListener { _, _, _, _ -> isFormChanged = true }
            }
        }
    }
    ...
    private fun getCheckedTextsFromLayout(container: LinearLayout): String {
        ...
        for (i in 0 until container.childCount) {
            ...
            if (view is CheckBox && view.isChecked) selected.add(view.text.toString())
        }
    ...
    private fun simpanLaporan() {
        ...
        lifecycleScope.launch {
            val laporan = LaporanGawatDarurat(
            ...
            val newId: Long = withContext(Dispatchers.IO) {
                db.laporanDao().insert(laporan)   // pakai laporanDao()
            }
            ...
            runOnUiThread {
                Toast.makeText(this@InputGawatDaruratActivity, "Laporan tersimpan", Toast.LENGTH_SHORT).show()
                finish()
            }
        }
    }
